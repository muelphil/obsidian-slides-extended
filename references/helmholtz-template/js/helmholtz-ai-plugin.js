...
const HelmholtzAIPlugin = (() => {
  ...
  /**
   * Parse slide properties from either:
   *  1. OSE-style <!-- .slide: data-key="value" --> comment (generated by SlideYamlProcessor
   *     from backtick yaml fences — the format used inside Obsidian Slides Extended).
   *  2. Original YAML front matter  ---\nkey: value\n---\n  (used by standalone index.html).
   *
   * Returns { meta: {key:value, …}, content: 'remaining markdown' } or null.
   *
   * Key mapping when reading data-* attributes back to meta:
   *   data-background-image → bg   (SlideYamlProcessor converts bg→data-background-image)
   *   data-<key>            → <key> (strip the data- prefix for all others)
   */
  function _parseFrontMatter(slideText) {
    // ── Format 1: OSE <!-- .slide: data-layout="..." ... --> ──────────────────
    const slideCommentMatch = slideText.match(/^<!--\s*\.slide:([\s\S]*?)-->\n?([\s\S]*)$/);
    if (slideCommentMatch) {
      const meta = {};
      const attrStr = slideCommentMatch[1];

      // Parse every data-key="value" attribute, stripping the data- prefix.
      // Special case: data-background-image → bg (matches SlideYamlProcessor's bg→data-background-image mapping)
      const dataAttrRegex = /\bdata-([a-zA-Z][a-zA-Z0-9-]*)="([^"]*)"/g;
      let m;
      while ((m = dataAttrRegex.exec(attrStr)) !== null) {
        const key = m[1] === 'background-image' ? 'bg' : m[1];
        meta[key] = m[2];
      }
      // Also extract plain class="..." attribute
      const classMatch = attrStr.match(/\bclass="([^"]*)"/);
      if (classMatch) meta.class = classMatch[1];

      // Only intercept when a layout key is present (otherwise nothing to transform)
      if (meta.layout) {
        return { meta, content: slideCommentMatch[2] || '' };
      }
      return null;
    }

    // ── Format 2: original --- YAML --- front matter ──────────────────────────
